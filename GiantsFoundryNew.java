...
import org.osbot.OP;
import org.osbot.rs07.accessor.XRS2Buffer;
...
public final class GiantsFoundryNew extends Script{
    ...
    RS2Widget forteWidget = null;
    RS2Widget bladeWidget = null;
    RS2Widget tipWidget = null;
    RS2Widget firstOption = null;
    RS2Widget secondOption = null;
    RS2Widget mouldScreen = null;
    ...
    Position kovac = new Position(3368,11487,0);
    ...
    boolean fortMould = false;
    boolean bladeMould = false;
    boolean tipBlade = false;
    boolean fortScreen = false;
    boolean bladeScreen = false;
    boolean tipScreen = false;
    boolean Open = false;
    ...
    public final void onStart() throws InterruptedException {
        ...
        fortes.add("Gladius Ricasso");
        fortes.add("Medusa Ricasso");
        fortes.add("Serpent Ricasso");
        fortes.add("Serrated Forte");

        blades.add("Defenders Blade");
        blades.add("Fish Blade");
        blades.add("Gladius Edge");
        blades.add("Medusa Blade");
        blades.add("Saw Blade");
        blades.add("Stiletto Blade");

        tips.add("Chopper Tip");
        tips.add("Gladius Point");
        tips.add("Medusa's Head");
        tips.add("People Poker Point");
        tips.add("Saw Tip");
        tips.add("Serpent's Fang");
    ...
    public final int onLoop() throws InterruptedException {
        ...
        else
        {
            while(configs.get(3430) == 0 && !Open)
            {
                ...
                Sleep2.sleepUntil(() -> getWidgets().getWidgetContainingText("Giants' Foundry Mould Setup") != null, 10000);
                Open = true;
            }
            String reqOneString = "";
            String reqTwoString = "";
            ...
            while (bar < 2 && configs.get(3430) != 2)
            {
                ...
                if (inventory.contains(bar1))
                {
                    ...
                    Sleep2.sleepUntil(() -> getWidgets().getWidgetContainingText("What metal would you like to add?") != null, 10000);
                ...
                if (inventory.contains(bar2))
                {
                    ...
                    Sleep2.sleepUntil(() -> getWidgets().getWidgetContainingText("How metal would you like to add?") != null, 10000);
    ...
    public void setWidgets() {
        try {
            ...
            /*
            heatProgressCold = findWidgets(754, 19);
            heatProgressWarm = findWidgets(754, 20);
            heatProgressHot = findWidgets(754, 21);
            heatArrow = findWidgets(754, 74);
            progressArrow = findWidgets(754, 78);
            progressOne = findWidgets(754, 75, 0);
            progressTwo = findWidgets(754, 75, 11);
            progressThree = findWidgets(754, 75, 22);
            progressFour = findWidgets(754, 75, 33);
             */
    ...
    public RS2Widget findWidgets(int childOne, int childTwo, int childThree)
    {
        RS2Widget storedWidget = getWidgets().get(childOne, childTwo, childThree);
        return storedWidget;
    }

    public RS2Widget findWidgets(int childOne, int childTwo)
    {
        RS2Widget storedWidget = getWidgets().get(childOne, childTwo);
        return storedWidget;
    }
